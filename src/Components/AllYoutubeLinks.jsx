...
export default function AllYoutubeLinks() {
  ...
  const getBlog = async () => {
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_URL}/youtube/youtube-links`
      );
      setImage(res.data.YUrl);
  ...
  return (
    <div className="flex-1 py-10 flex flex-col justify-between">
      <div className="w-full md:p-10 p-4">
        <h2 className="pb-4 text-lg font-medium">All Youtube Link</h2>

        {loading ? (
          <p>Loading Youtube Url...</p>
        ) : (
